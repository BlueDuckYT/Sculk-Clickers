...
public class ForgeEvents {
            ...
            if (Config.apparitionCatalystSpawnChance > 0) {
                if (nearSculkCatalyst(event.getEntity().level(), event.getEntity())) {
                    if (event.getEntity().getRandom().nextDouble() < Config.apparitionCatalystSpawnChance) {
                        Apparition apparition = BlightEntities.SCULK_APPARITION.get().create(serverLevel, EntitySpawnReason.EVENT);
                        if (apparition != null) {
                            apparition.setPos(event.getEntity().getX(), event.getEntity().getY() - 2.25D, event.getEntity().getZ());
                            apparition.setYRot(event.getEntity().getYRot());
                            apparition.setXRot(event.getEntity().getXRot());
                            serverLevel.addFreshEntityWithPassengers(apparition);
    ...
    public static void entityTickEvent(PlayerTickEvent.Pre event) {
        if (!event.getEntity().level().isClientSide() && ModList.get().isLoaded("sculksickness")) {
            var sculkSicknessEffectOpt = BuiltInRegistries.MOB_EFFECT.get(ResourceLocation.fromNamespaceAndPath("sculksickness", "sculk_sickness"));
            if (sculkSicknessEffectOpt.isPresent()) {
                var sculkSicknessHolder = sculkSicknessEffectOpt.get();
                if (event.getEntity().hasEffect(sculkSicknessHolder)) {
                    int amplifier = -1;
                    for (MobEffectInstance effect : event.getEntity().getActiveEffects()) {
                        if (effect.getEffect().equals(sculkSicknessHolder)) {
                    ...
                    if (event.getEntity().getRandom().nextDouble() < Config.apparitionSicknessSpawnChance * (amplifier + 1)) {
                        ...
                        Apparition apparition = BlightEntities.SCULK_APPARITION.get().create(serverLevel, EntitySpawnReason.EVENT);
                        if (apparition != null) {
                            apparition.setPos(event.getEntity().getX() + xOffset, event.getEntity().getY() + yOffset, event.getEntity().getZ() + zOffset);
                            apparition.setYRot(event.getEntity().getYRot());
                            apparition.setXRot(event.getEntity().getXRot());
                            serverLevel.addFreshEntityWithPassengers(apparition);
                        }
