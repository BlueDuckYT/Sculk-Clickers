...
import blueduck.blighted_beasts.client.renderer.SkitterRenderState;
...
public class SkitterModel extends EntityModel<SkitterRenderState> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(BlightedBeasts.MODID, "skitter"), "main");
    ...
    @Override
    public void setupAnim(SkitterRenderState state) {
        ...
        if (state.isSwinging) {
            head.z = (float) (Math.sin(state.attackAnimProgress * 6.28) * -6.0F  - 6.0F);
        }
        else {
            head.z = -6F;
        }
