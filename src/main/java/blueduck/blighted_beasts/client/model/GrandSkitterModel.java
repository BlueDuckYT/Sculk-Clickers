...
public class GrandSkitterModel<T extends GrandSkitter> extends HierarchicalModel<T> {
    ...
    public GrandSkitterModel(ModelPart root) {
    ...
    @Override
    public void setupAnim(GrandSkitter entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
...
//		this.animate(entity.runAnimationState, SkitterAnimation.run, ageInTicks);
        ...
        leg0.xRot = (float) (Math.sin(limbSwing) * Math.sqrt(limbSwingAmount)/2);
        leg1.xRot = (float) (-Math.sin(limbSwing) * Math.sqrt(limbSwingAmount)/2);
        leg2.xRot = (float) (Math.cos(limbSwing) * Math.sqrt(limbSwingAmount)/2);

        body.y = (float) Math.sin(ageInTicks * 0.1 + 3)  * 0.25F - 24.0F;

        float scale = (float) Math.abs(Math.sin(ageInTicks * 0.1 + 8)) * 0.02F + 1;
        body.xScale = scale;
        body.yScale = scale;
        body.zScale = scale;

        head.xRot = (float) Math.cos(ageInTicks * 0.05) * 0.05F + .1745F;
