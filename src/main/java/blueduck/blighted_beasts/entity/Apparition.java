...
import software.bernie.geckolib.animatable.processing.AnimationController.AnimationStateHandler;
...
public class Apparition extends Vex implements GeoEntity, VibrationSystem {
    ...
    private PlayState predicate(AnimationTest<Apparition> state) {
        if (this.getTarget() == null || this.getTarget().isDeadOrDying()) {
            ...
            state.controller().setAnimation(RawAnimation.begin().thenLoop("animation.model.move"));
        }
        return PlayState.CONTINUE;
    }

    private PlayState attackPredicate(AnimationTest<Apparition> state) {
        if (this.swinging && this.swingTime != -1) {
            state.controller().setAnimation(RawAnimation.begin().thenPlay("animation.model.attack"));
            return PlayState.CONTINUE;
        }
