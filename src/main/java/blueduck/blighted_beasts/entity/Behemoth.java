...
public class Behemoth extends Monster implements GeoEntity, VibrationSystem {
    ...
    private PlayState predicate(AnimationState state) {
        if (state.isMoving() && ((this.getXZSpeed() > .325D || (this.isTargeting && (this.getTarget() != null && !this.getTarget().isDeadOrDying()))))) {
    ...
    class BehemothMeleeAttackGoal extends MeleeAttackGoal {
        ...
        protected double getAttackReachSqr(LivingEntity p_33377_) {
            float f = Behemoth.this.getBbWidth() - 0.1F;
            return (double)(f * 1.25F * f * 1.25F);
        }
    }
}
